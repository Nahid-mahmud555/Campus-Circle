    ...
    if (descriptionInput && aiFeedback) {
        descriptionInput.addEventListener('input', function() {
            if (this.value.length > 20) {
                ...
                aiFeedback.textContent = 'Good start! Consider adding more details.';
            } else if (this.value.length > 0) {
                aiFeedback.textContent = 'Add more details to make your listing stand out';
            } else {
...
function initializeDepartmentSelection() {
    const allDepartmentsCheckbox = document.querySelector('input[name="targetDepartments"][value="all"]');
    const departmentCheckboxes = document.querySelectorAll('input[name="targetDepartments"]:not([value="all"])');
...
function getSelectedDepartments() {
    ...
    const selected = [];
    const departmentCheckboxes = document.querySelectorAll('input[name="targetDepartments"]:checked');
    departmentCheckboxes.forEach(checkbox => {
...
function handleUpload(e) {
    ...
    if (!title || !description || price <= 0 || selectedDepartments.length === 0) {
...
function getDefaultImage(category) {
    const defaultImages = {
        ...
        'academic': 'https://images.unsplash.com/photo-1581726707445-75cbe4efc586?w=400&h=250&fit=crop',
        ...
        'language': 'https://images.unsplash.com/photo-1523580494863-6f3031224c94?w=400&h=250&fit=crop'
    };
    
    return defaultImages[category] || 'https://images.unsplash.com/photo-1498837167922-ddd27525d352?w=400&h=250&fit=crop';
}
...
function loadMarketplaceCourses() {
    ...
    marketplaceGrid.innerHTML = sampleCourses.map(course => `
        <div class="skill-card" data-category="${course.category}" data-title="${course.title.toLowerCase()}">
            ${course.badge ? `<div class="skill-badge">${course.badge}</div>` : ''}
            <div class="skill-image" style="background-image: url('${course.image}')"></div>
            <div class="skill-content">
                ...
                <div class="skill-actions">
                    ...
                    <button class="info-btn" onclick="showCourseDetails(${course.id})">
...
function bookCourse(courseId) {
    ...
    if (course) {
        showSuccess(`Booked: ${course.title} for ${course.price}৳`);
    }
}
...
function generateAIResponse(userMessage) {
    ...
    if (message.includes('trend') || message.includes('popular')) {
        return `Based on current data, these skills are trending at Varendra University:
...
💡 **Tip:** Consider learning these to increase your earning potential!`;
    
    } else if (message.includes('sell') || message.includes('course') || message.includes('teach')) {
        return `**How to Sell Your Skills/Courses:**

1. **Complete Verification** - First, verify your student status
2. **Create Listing** - Go to Dashboard → Sell Skill
3. **Set Fair Price** - AI will suggest optimal pricing (50-100৳ recommended)
4. **Add Clear Description** - Explain what students will learn
5. **Upload Demo** - Add images/videos to showcase your work

💰 **Commission:** Only 15% per successful transaction!
🎯 **Popular Categories:** Programming, Design, Language, Business`;
    
    } else if (message.includes('campus') || message.includes('event')) {
        return `**Upcoming Campus Events:**

🗓️ **This Week:**
• Tech Seminar: AI in Education (Tomorrow 3 PM)
• Midterm Exams Begin (Next Week)
• Coding Competition (Friday)
• Career Fair (Next Month)

🎯 **Club Activities:**
• Programming Club - Web Dev Workshop
• Business Club - Startup Pitch Session
• Cultural Club - Music Practice
• Sports Club - Tournament

📚 **Academic:**
• Library extended hours during midterms
• Research paper submission deadline approaching`;
    
    } else if (message.includes('buy') || message.includes('purchase') || message.includes('book')) {
        return `**Buying Guide:**
...
💡 **Tip:** Look for courses with high ratings and detailed descriptions!`;
    
    } else if (message.includes('hello') || message.includes('hi') || message.includes('hey')) {
        return `Hello! I'm your Campus AI Assistant. I can help you with:

🤝 **Buying/Selling Skills**
...
🎯 **Learning & Teaching:**
• Find the right courses to learn
• Set up your own skill sessions
...
function formatAIResponse(text) {
    return text.split('\n\n').map(paragraph => {
        if (paragraph.includes('**') && paragraph.includes(':**')) {
            return `<strong>${paragraph.replace(/\*\*(.*?)\*\*/g, '$1')}</strong>`;
        } else if (paragraph.includes('•')) {
            const items = paragraph.split('•').filter(item => item.trim());
            return items.map(item => `<div style="margin: 2px 0;">• ${item.trim()}</div>`).join('');
        } else if (paragraph.includes('🎯') || paragraph.includes('📈') || paragraph.includes('🛒') || paragraph.includes('🗓️') || paragraph.includes('🔒') || paragraph.includes('💡') || paragraph.includes('📚') || paragraph.includes('🎓') || paragraph.includes('📱') || paragraph.includes('🏫') || paragraph.includes('🤝')) {
...
function filterNewsfeedPosts(query) {
    ...
    posts.forEach(post => {
        ...
        if (title.includes(lowerQuery) || description.includes(lowerQuery) || category.includes(lowerQuery) || query === '') {
...
function updateUserStats() {
    ...
    if (skillsCount) skillsCount.textContent = userPosts.filter(post => post.type === 'skill').length;
            ...
            <div class="post-content">
                ...
                <div class="post-meta">
                    ${post.type === 'skill' && post.price > 0 ? 
                        `<span class="post-price">${post.price} ৳</span>` : 
                        `<span class="post-price">${post.type === 'skill' ? 'Free' : 'Help Request'}</span>`
                    }
                    <span class="post-category">${getCategoryName(post.category)}</span>
                </div>
            </div>
...
function updateCommission() {
    ...
    if (priceInput && earningsAmount && totalAmount && commissionAmount) {
        ...
        if (commissionMessage) {
            commissionMessage.textContent = `Platform fee: 15% (${commission}৳) will be deducted`;
        }
    }
}
